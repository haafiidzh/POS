...
<div x-data="{ {{ $filterId }}: false }" id="{{ $filterId }}"
    ...
    <button x-on:click="{{ $filterId }} = ! {{ $filterId }}"
            class="rounded-full bg-slate-200 hover:bg-slate-300 dark:hover:bg-gray-800 h-4 w-4 p-0 flex justify-center items-center ms-1">
        <i class="bx bx-chevron-down text-base"></i>
    </button>

    @isset($query)
        @if ($query)
            <a class="leading-none rounded-full bg-slate-200 hover:bg-slate-300 dark:hover:bg-gray-800 h-4 w-4 p-0 flex justify-center items-center ms-1"
