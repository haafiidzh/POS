<div x-show="editModal" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    ...
    <div class="flex items-end justify-center min-h-screen px-4 text-center items-center sm:p-0">
        ...
        <div x-cloak x-show="editModal" x-transition:enter="transition ease-out duration-300 transform"
            ...
            <form class="p-6 {{ $loading ? 'skeleton' : null }}" wire:submit.prevent="update"
                  wire:loading.class="skeleton" wire:target="update">
                <div class="grid md:grid-cols-2 gap-3">
                    {{-- Category --}}
                    <div class="form-group md:w-8/12">
                        <label for="category">Kategori</label>
                        <select class="form-input" id="category" wire:model.defer="category">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="question" wire:model.defer="question">
                    @error('question')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea type=" text" class="form-input" id="answer" wire:model.defer="answer"></textarea>
                    @error('answer')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                @can('publish.faq', 'web')
                    ...
                    <div class="form-group ">
                        <div class="custom-control custom-switch">
                            ...
                            <label class="custom-control-label" for="is_active">Aktifkan FAQ?</label>
                            <br>
                            @error('is_active')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                ...
                <div class="form-group ">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="form-checkbox rounded text-primary" id="is_featured"
                               name="is_featured" wire:model.defer="is_featured">
                        <label class="custom-control-label" for="is_featured">Unggulan?</label>
                        <br>
                        @error('is_featured')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
